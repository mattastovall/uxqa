...
function Icon({ kind }: Readonly<{ kind: "back" | "close" | "more" | "share" | "tabs" | "refresh" | "lock" }>) {
  ...
  return <svg viewBox="0 0 24 24" focusable="false">{paths[kind]}</svg>;
}
...
function StatusBar({ profile }: Readonly<{ profile: ResolvedSimulatorProfile }>) {
  ...
  return <div className="uxqa-chrome-status" style={{ height }}><span>9:41</span>{profile.device.id === "iphone-16" ? <span className="uxqa-dynamic-island" /> : null}<span className="uxqa-status-indicators">▮ ◔ ▰</span></div>;
}

function Address({ hostname }: Readonly<{ hostname: string }>) {
  return <span className="uxqa-address-pill"><span className="uxqa-glyph"><Icon kind="lock" /></span><span className="uxqa-hostname">{hostname}</span><span className="uxqa-glyph"><Icon kind="refresh" /></span></span>;
}
...
function Desktop({ hostname, windows }: Readonly<{ hostname: string; windows: boolean }>) {
  return <div className={`uxqa-top-chrome ${windows ? "uxqa-windows-chrome" : "uxqa-macos-safari"}`}><div className="uxqa-desktop-tab-row">{windows ? null : <WindowControls />}<span className="uxqa-desktop-tab">Website preview</span><span className="uxqa-new-tab">+</span>{windows ? <WindowControls /> : null}</div><div className="uxqa-desktop-address-row"><span className="uxqa-glyph"><Icon kind="back" /></span>{windows ? <span className="uxqa-glyph"><Icon kind="refresh" /></span> : null}<Address hostname={hostname} /><span className="uxqa-glyph"><Icon kind="more" /></span></div></div>;
}
